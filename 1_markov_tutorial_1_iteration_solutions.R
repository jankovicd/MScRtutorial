...
r_HD    <- - log(1 - df_psa_1$p_HD) 
# rate of death in sick
r_S1D   <- df_psa_1$hr_S1 * r_HD 	  
# rate of death in sicker
r_S2D   <- df_psa_1$hr_S2 * r_HD  
...
### From Sick
m_P["S1", "H"]  <- df_psa_1$p_S1H
m_P["S1", "S1"] <- 1 - (df_psa_1$p_S1H + df_psa_1$p_S1S2 + p_S1D)
m_P["S1", "S2"] <- df_psa_1$p_S1S2
m_P["S1", "D"]  <- p_S1D
### From Sicker
m_P["S2", "S2"] <- 1 - p_S2D
m_P["S2", "D"]  <- p_S2D
...
v_c_trt    <- c(df_psa_1$c_H, df_psa_1$c_S1 + df_psa_1$c_Trt,
                df_psa_1$c_S2 + df_psa_1$c_Trt, df_psa_1$c_D)
